...
#######################
GITLAB_URL = 'https://gitlab.geant.net'
GITLAB_TOKEN = 'xxxxxx'

# needed to clone the repositories, keep empty to try publickey (untested)
GITLAB_ADMIN_USER = 'root'
GITLAB_ADMIN_PASS = 'xxxxxxx'

GITEA_URL = 'https://test-code.geant.org'
GITEA_TOKEN = 'xxxxxx'
#######################
# CONFIG SECTION END
#######################
...
def get_issues(gitea_api: pygitea, owner: string, repo: string) -> []:
    ...
    if issue_response.ok:
        ...
        print_error(f"Failed to load existing issues for project {repo}! {issue_response.text}")
...
def get_teams(gitea_api: pygitea, orgname: string) -> []:
    ...
    if team_response.ok:
        ...
        print_error(
            f"Failed to load existing teams for organization {orgname}! {team_response.text}"
        )
...
def get_collaborators(gitea_api: pygitea, owner: string, repo: string) -> []:
    ...
    collaborator_response: requests.Response = gitea_api.get(f"/repos/{owner}/{repo}/collaborators")
...
def user_key_exists(gitea_api: pygitea, username: string, keyname: string) -> bool:
    ...
    if existing_keys:
        ...
        print(f"Public key {keyname} does not exists for user {username}, importing!")
        return False
    else:
...
def repo_exists(gitea_api: pygitea, owner: string, repo: string) -> bool:
    ...
    repo_response: requests.Response = gitea_api.get(f"/repos/{owner}/{repo}")
    if repo_response.ok:
        print_warning(f"Project {repo} does already exist in Gitea, skipping!")
    else:
        print(f"Project {repo} not found in Gitea, importing!")
...
def label_exists(gitea_api: pygitea, owner: string, repo: string, labelname: string) -> bool:
    ...
    if existing_labels:
        ...
        if existing_label is not None:
            print_warning(f"Label {labelname} already exists in project {repo}, skipping!")
            return True

        print(f"Label {labelname} does not exists in project {repo}, importing!")
        return False
...
def milestone_exists(gitea_api: pygitea, owner: string, repo: string, milestone: string) -> bool:
    ...
    if existing_milestones:
        ...
        if existing_milestone is not None:
            print_warning(f"Milestone {milestone} already exists in project {repo}, skipping!")
            return True

        print(f"Milestone {milestone} does not exists in project {repo}, importing!")
        return False
...
def issue_exists(gitea_api: pygitea, owner: string, repo: string, issue: string) -> bool:
    ...
    if existing_issues:
        ...
        print(f"Issue {issue} does not exists in project {repo}, importing!")
        return False
    ...
    for issue in issues:
        if not issue_exists(gitea_api, owner, repo, issue.title):
            ...
            if issue.milestone is not None:
                existing_milestone = next(
                    (
                        item for item in existing_milestones if item["title"] == issue.milestone['title']
                    ), None
                )
                if existing_milestone:
            ...
            import_response: requests.Response = gitea_api.post(
                f"/repos/{owner}/{repo}/issues", json={
                    "assignee": assignee,
                    "assignees": assignees,
...
def _import_users(gitea_api: pygitea, users: [gitlab.v4.objects.User], notify: bool = False):
    for user in users:
        ...
        if not user_exists(gitea_api, user.username):
            ...
            tmp_email = f"{user.username}@noemail-git.local"  # Some gitlab instances do not publish user emails
            try:
            ...
            if import_response.ok:
                print_info(f"User {user.username} imported, temporary password: {tmp_password}")
            else:
...
def _import_user_keys(gitea_api: pygitea, keys: [gitlab.v4.objects.UserKey], user: gitlab.v4.objects.User):
    ...
    for key in keys:
        if not user_key_exists(gitea_api, user.username, key.title):
            import_response: requests.Response = gitea_api.post(
                f"/admin/users/{user.username}/keys", json={
                    "key": key.key,
                    "read_only": True,
                    "title": key.title,
                }
...
def name_clean(name):
    """Cleans a name for usage in Gitea"""
